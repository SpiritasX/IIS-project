...
function StockBarChart({ data, loading }) {
  if (loading) {
    return (
      <div className="dashboard-chart-card">
        <h3 className="dashboard-chart-title">Stock levels</h3>
  ...
  const maxVal = Math.max(...data.map((d) => d.inStock ?? 0), 1)
  ...
  return (
    <div className="dashboard-chart-card">
      <h3 className="dashboard-chart-title">Stock levels</h3>
      <svg
        aria-label="Stock levels bar chart"
      ...
      >
        ...
        {data.map((d, i) => {
          const barH = Math.max(((d.inStock ?? 0) / maxVal) * chartH, 1)
          ...
          return (
            <g key={d.plantName + i}>
              ...
              >
                {d.varietyName?.split(' ').pop() ?? d.plantName?.split(' ')[0]}
              </text>
            </g>
          )
        })}
      </svg>
      <span className="dashboard-chart-legend">Units in active stock per plant</span>
    </div>
  )
}
